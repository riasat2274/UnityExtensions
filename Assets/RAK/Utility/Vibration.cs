...
namespace RAK
{
    public class Vibration
    {
        ...
        {
            if (!GlobalToggle.IsEnabled(GlobalToggleType.Vibration)) return ;
            if (isAndroid())
            {
                //If Android 8.0 (API 26+) or never use the new vibrationeffects
                if (getSDKInt() >= 26)
        ...
        public static void CreateOneShotMax(float duration)
        {
            ...
            {
                // Check if the device is a valid haptic device
                if (device.TryGetHapticCapabilities(out HapticCapabilities capabilities))
                {
                    if (capabilities.supportsImpulse)
                    {
                        // Send haptic impulse with maximum amplitude (1.0) and a short duration
                        // Duration can be adjusted as needed
                        device.SendHapticImpulse(uint.MaxValue, 1.0f, duration); // 1.0f is max amplitude, 0.5f is duration in seconds
                        //Debug.Log("Triggered max haptic feedback on a device.");
                    }
                }
            }
        }
